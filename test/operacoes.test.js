...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => { 
    ...
    // Verifica pela referencia dos objetos
    const valor = { valueOf: () => 5 };
    const min = { valueOf: () => 5 };
    const max = { valueOf: () => 10 };
    // quando valor == min a implementação original retorna o próprio `valor` (não `min`)
    expect(clamp(valor, min, max)).toBe(valor);

    const valorMax = { valueOf: () => 10 };
    const min2 = { valueOf: () => 5 };
    const max2 = { valueOf: () => 10 };
    // quando valor == max a implementação original retorna o próprio `valor` (não `max`)
    expect(clamp(valorMax, min2, max2)).toBe(valorMax);

    // Verificacasos onde o valor fica fora do intervalo. Deve retornar min/max (referência específica)
    const low = { valueOf: () => 0 };
    const high = { valueOf: () => 20 };
    const min3 = { valueOf: () => 5 };
    const max3 = { valueOf: () => 10 };
  ...
  test('38. deve converter Celsius para Fahrenheit', () => { 
    ...
    expect(celsiusParaFahrenheit(15)).toBeCloseTo(59);
  });
  test('39. deve converter Fahrenheit para Celsius', () => { 
    expect(fahrenheitParaCelsius(32)).toBe(0); 
    expect(fahrenheitParaCelsius(59)).toBeCloseTo(15);
  });
  ...
  test('47. mediana: array ímpar ordenado', () => { 
    expect(medianaArray([1, 2, 3, 4, 5])).toBe(3); 
    expect(medianaArray([7, 1, 3, 9])).toBe(5);
